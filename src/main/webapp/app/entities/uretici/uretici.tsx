...
import { getEntities } from './uretici.reducer';
...
export const Uretici = (props: IUreticiProps) => {
  ...
  return (
    <div>
      <h2 id="uretici-heading" data-cy="UreticiHeading">
        <Translate contentKey="koopApp.uretici.home.title">Ureticis</Translate>
        <div className="d-flex justify-content-end">
          <Button className="mr-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="koopApp.uretici.home.refreshListLabel">Refresh List</Translate>
          </Button>
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="koopApp.uretici.id">ID</Translate> <FontAwesomeIcon icon="sort" />
                </th>
            ...
            <tbody>
              {ureticiList.map((uretici, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{uretici.tarih ? <TextFormat type="date" value={uretici.tarih} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{uretici.user ? uretici.user.login : ''}</td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`${match.url}/${uretici.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${uretici.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${uretici.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
